...
/*   Updated: 2024/12/02 17:08:49 by sjoukni          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	c_padding(int num_len, t_flags *flags, int precision_padding)
{
	int	padding;
	int	padding_type;

	padding = flags->width - num_len - precision_padding;
	if (num_len < 0 || flags->plus_sign || flags->space_sign)
		padding--;
...
int	handle_integer(va_list args, t_flags *flags)
{
	...
	if (len > 0)
		return (len);
	space_padding = c_padding(num_len, flags, precision_padding);
